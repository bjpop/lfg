...
-- It uses exactly the same initialisation and generation algorithm
-- as version 4 of the Scalable Parallel Pseudo Random Number Generator
-- library (SPRNG) http://sprng.cs.fsu.edu/. It allows new independent
-- generators to be spawned from existing ones a large (but not
-- infinite) number of times.
--
-- New generators are created using the init function:
--    init gen totalGen seed param
--
-- or, given an existing generator, using the spawn function:
--    spawn gen numSpawn
--
-- The interface to the generator is overloaded by the Variate class,
-- which provides the functions:
-- uniform, and uniformR.

-- Warning: the implementation of the generator is a direct translation
-- of the C++ code from SPRNG. As such it is quite complicated, containing
-- lots of low-level bit manipulation. Unfortunately the original SPRNG code
-- was not well documented, and this Haskell implementation shares that
-- quality. Our goal was to reproduce exactly the sequence of bits
-- generated by SPRNG. Without a full understanding of how the original
-- code works, our only option was to slavishly imitate the code.
...
-- A vector containing elements of type Word32
...
init :: PrimMonad m => Word32 -> Int -> m (Gen (PrimState m))
init = initOneOfMany 0 1

-- XXX some sanity checking of inputs needs to be done
initOneOfMany :: PrimMonad m => Int -> Int -> Word32 -> Int -> m (Gen (PrimState m))
-- init :: Int -> Int -> Word32 -> Int -> IO (Gen (PrimState IO))
initOneOfMany {- gen -} g {- total gen -} tg {- seed -} s {- parameter -} pa = do
...
spawn :: PrimMonad m => Gen (PrimState m) -> Int -> m [Gen (PrimState m)]
spawn gen nspawned = do
   let p = gen_si gen
